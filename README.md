...
`db:setup` создаёт базу, загружает схему и заполняет три демо-сценария (вывод в консоль покажет что произошло).
...
erDiagram
    ...
    JournalEntry ||--|{ Posting : groups
    JournalEntry ||--o| JournalEntry : reverses
    Order }o--o| JournalEntry : "paid_journal_entry"
    Order }o--o| JournalEntry : "cancellation_journal_entry"
```
